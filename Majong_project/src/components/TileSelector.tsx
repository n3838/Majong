...
const TileSelector: React.FC<TileSelectorProps> = ({ onTileSelect, hand }) => {
  ...
  const getTileCount = (type: TileType, value: number | HonorType): number => {
    return hand.filter(tile => tile.type === type && tile.value === value).length;
  };
      ...
      tiles.push(
        <div key={`${type}-${i}`} className="relative">
          <TileComponent
            ...
            size="small"
            className={`m-1 ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110'}`}
          />
          {count > 0 && (
      ...
      return (
        <div key={`honor-${honor}`} className="relative">
          <TileComponent
            ...
            onClick={() => !isDisabled && onTileSelect(createTile('honor', honor))}
            size="small"
            className={`m-1 ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110'}`}
          />
          {count > 0 && (
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-800">牌を選択 / Select Tiles</h3>
      ...
      <div className="space-y-4">
        {(selectedSuit === 'all' || selectedSuit === 'man') && (
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-2">萬子 (Characters)</h4>
            <div className="flex flex-wrap">
              {renderNumberTiles('man')}
            </div>
          </div>
        )}
        ...
        {(selectedSuit === 'all' || selectedSuit === 'pin') && (
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-2">筒子 (Circles)</h4>
            <div className="flex flex-wrap">
              {renderNumberTiles('pin')}
            </div>
          </div>
        )}
        ...
        {(selectedSuit === 'all' || selectedSuit === 'sou') && (
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-2">索子 (Bamboo)</h4>
            <div className="flex flex-wrap">
              {renderNumberTiles('sou')}
            </div>
          </div>
        )}
        ...
        {(selectedSuit === 'all' || selectedSuit === 'honor') && (
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-2">字牌 (Honor Tiles)</h4>
            <div className="flex flex-wrap">
              {renderHonorTiles()}
            </div>
          </div>
