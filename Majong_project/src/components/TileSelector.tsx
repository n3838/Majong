...
const TileComponent: React.FC<TileComponentProps> = ({
  ...
  // 新しいファイル名形式に合わせて画像URLを生成するロジック
  const getImageUrl = () => {
    // 'man' -> 'Man', 'honor' -> 'Honor'のように先頭を大文字にする
    const type = tile.type.charAt(0).toUpperCase() + tile.type.slice(1);

    if (tile.type === "honor") {
      // 'east' -> 'East'のように先頭を大文字にする
      const value =
        String(tile.value).charAt(0).toUpperCase() +
        String(tile.value).slice(1);
      return `/Black/${type}-${value}.png`; // 例: /PNG/Honor-East.png
    } else {
      const value = tile.value;
      return `/Black/${type}${value}.png`; // 例: /PNG/Man1.png
    }
  };
